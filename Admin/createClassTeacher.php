...
<head>
  ...
  <script>
    function classArmDropdown(classId) {
      if (classId == "") {
          document.getElementById("classArmSelect").innerHTML = "<select required name='classArmId' class='form-control mb-3'><option value=''>--Select Class First--</option></select>";
          return;
      } else { 
          var xmlhttp;
          ...
          xmlhttp.onreadystatechange = function() {
              if (this.readyState == 4 && this.status == 200) {
                  document.getElementById("classArmSelect").innerHTML = this.responseText;
              }
          };
    ...
    window.onload = function() {
      <?php if(isset($row['classId'])): ?>
        classArmDropdown('<?php echo $row['classId']; ?>');
      <?php endif; ?>
    }
  </script>
</head>
                  ...
                  <form method="post">
                   <div class="form-group row mb-3">
                     ...
                     <div class="form-group row mb-3">
                        ...
                        <input type="text" class="form-control" name="phoneNo" value="<?php echo isset($row['phoneNo']) ? $row['phoneNo'] : ''; ?>" id="exampleInputPhone" >
                        </div>
                    </div>
                    <div class="form-group row mb-3">
                        <div class="col-xl-6">
                            ...
                            $num = $result->num_rows;		
                            if ($num > 0){
                                echo '<select required name="classId" onchange="classArmDropdown(this.value)" class="form-control mb-3" id="classIdSelect">';
                        ...
                        <div id="classArmSelect">
                            <?php
                            if(isset($row['classId'])) {
                                $classId = $row['classId'];
                                $classArmId = isset($row['classArmId']) ? $row['classArmId'] : '';
                                $qryArms = "SELECT * FROM tblclassarms WHERE classId='$classId'";
                                $resultArms = $conn->query($qryArms);
                                echo '<select required name="classArmId" class="form-control mb-3">';
                                echo '<option value="">--Select Section--</option>';
                                while($arm = $resultArms->fetch_assoc()) {
                                    $selected = ($classArmId == $arm['Id']) ? 'selected' : '';
                                    echo '<option value="'.$arm['Id'].'" '.$selected.'>'.$arm['classArmName'].'</option>';
                                }
                                echo '</select>';
                            } else {
                                echo '<select required name="classArmId" class="form-control mb-3"><option value="">--Select Class First--</option></select>';
                            }
                            ?>
                        </div>
                        </div>
                    ...
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <a href="createClassTeacher.php" class="btn btn-secondary">Cancel</a>
                    <?php
                    } else {           
                    ?>
                        ...
                        while ($rows = $rs->fetch_assoc())
                          {
                            ...
                            echo"
                              <tr>
                                ...
                                <td><a href='?action=delete&Id=".$rows['Id']."&classArmId=".$rows['classArmId']."' onclick=\"return confirm('Are you sure you want to delete this teacher?');\"><i class='fas fa-fw fa-trash'></i></a></td>
                              </tr>";
                          }
                      }
                      else
